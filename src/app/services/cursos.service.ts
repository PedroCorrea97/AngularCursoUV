...
export class CursosService {
  ...
  addCurso(payload:Cursos): Observable<Cursos>{
    console.log(payload);
    return this.http.post<Cursos>(`${this.apiURL}/cursos`, payload).pipe(catchError(this.handleError)) }

  update(id:number, payload: Cursos): Observable<Cursos>{ 
    return this.http.put<Cursos>(`${this.apiURL}/cursos/${id}`, payload).pipe(catchError(this.handleError)) }
